...
def test_add_record():
    ...
    to the custom singly linked list in memory.

    This test confirms:
    - A record is successfully inserted into the linked list.
    - The stored record fields match the expected values.
    """
    # Create instance of Business Layer that stores records in a custom singly linked list
    ...
    # Add the new record to the linked list structure in memory
    manager.add_record(record)

    # Verify the record count increased to 1
    assert len(manager.get_all_records()) == 1

    # Retrieve stored record from memory
    stored_record = manager.get_record(0)

    # Verify data was stored correctly in the record object
